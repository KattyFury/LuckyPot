...
export function DrawHistoryList({
  ...
  if (epochs.length === 0) {
    return <div style={{ fontSize: "var(--fs-1)", color: "var(--color-text-secondary)" }}>No draws yet.</div>;
  }
    ...
    <>
      {epochs.map(({ id, epoch }) => (
        <button key={id.toString()} className="card-list__row" onClick={() => onSelect(id, epoch)}>
          <span style={{ fontSize: "var(--fs-1)", fontWeight: 700 }}>Epoch #{id.toString().padStart(2, "0")}</span>
